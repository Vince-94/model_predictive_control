function [ x_LMPC, u_LMPC, x_cl_out, u_cl_out, IterationCost_out, SS] = LMPC(x0, x_cl, u_cl, IterationCost, A, B, Q, R, N, Iterations, X, U, LMPC_options, goalSet, x_bound, map, xf)
...
%         disp(['Time step: ', num2str(t), ', Iteration: ', num2str(j), ' Best Cost: ',num2str(IterationCost_out{j}(1))])
        ...
        if LMPC_options.goalSet == 0
            [~, uPred ] = FTOCP(x_LMPC(:,t), N, Q, R, Qfun, SS,...
                                                     A, B, X, U, LMPC_options, x_bound, map);
        else
...
%         end
    ...
    if abs(IterationCost_out{j}(1)-IterationCost_out{j-1}(1)) < 10e-2
        Exit_Loop = 1;
    end
