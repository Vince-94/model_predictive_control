function pred_model = bicycle_pred_model_II(x0, u0, sys, Ts, PointAndTangent)
...
%% A11
dvx_vx = -(2*Cf*delta_f*(vy + lf*wz))/(m*vx^2);
dvx_vy = wz + (2*Cf*delta_f)/(m*vx);
dvx_wz = vy + (2*Cf*delta_f*lf)/(m*vx);

dvy_vx = ((2*Cf*(vy + lf*wz))/vx^2 + (2*Cr*(vy - lr*wz))/vx^2)/m - wz;
dvy_vy = -((2*Cf)/vx + (2*Cr)/vx)/m;
dvy_wz = - vx - ((2*Cf*lf)/vx - (2*Cr*lr)/vx)/m;

dwz_vx = ((2*Cf*lf*(vy + lf*wz))/vx^2 - (2*Cr*lr*(vy - lr*wz))/vx^2)/Iz;
dwz_vy = -((2*Cf*lf)/vx - (2*Cr*lr)/vx)/Iz;
dwz_wz = -((2*Cf*lf^2)/vx + (2*Cr*lr^2)/vx)/Iz;                     % WARNING
...
%% B1
dvx_delta_f = -(2*Cf*(delta_f - (vy + lf*wz)/vx))/m - (2*Cf*delta_f)/m;
dvx_a = 1;

dvy_delta_f = (2*Cf)/m;
dvy_a = 0;

dwz_delta_f = (2*Cf*lf)/Iz;
dwz_a = 0;
