...
% state constraints [d_vx, d_vy, d_wz, d_s, d_ey, d_epsi]
x_c = [-1,          +1.5
       -0.2,        +0.2
       -30*pi/180,  +30*pi/180
       0,           +3
...
% input constraints [d_delta_f, d_a]
u_c = [-30*pi/180,  +30*pi/180
       -0.1,          +0.1];
...
while (flag == 0)
    ...
    fprintf('Np = %d, Nc = %d \n', [Np, Nc]);
    fprintf('Velocity =         [dvx = %.2fm/s, dvy = %.2fm/s,  dwz =   %.1f°/s] \n', [X_MPC(1,k), X_MPC(2,k), X_MPC(3,k)*180/pi]);
    fprintf('Curvilinear pose = [ds =  %.2fm,   dey = %.2fm,    depsi = %.1f°] \n',   [X_MPC(4,k), X_MPC(5,k), X_MPC(6,k)*180/pi]);
%     fprintf('Global pose =      [x =  %.2fm,    y =  %.2fm,     psi =  %.1f°] \n',   [X(4,k), X(5,k), X(6,k)*180/pi]);


    
    %% MPC
    [U_star] = adaptive_mpc(X_MPC(:,k), ref, U_old, T_ctrl, Np, Nc, Q, R, S, x_c, u_c, du_c, PointAndTangent, method);
...
%     delete(p1)
    ...
    X_MPC(:,k) = X_MPC(:,k) - X_MPC(:,k-1);
    
%     X_MPC(:,k) = [X(1,k)-X(1,k-1); X(2,k)-X(2,k-1); X(3,k)-X(3,k-1); s-X_MPC(4,k-1); ey-X_MPC(5,k-1); epsi-X_MPC(6,k-1)];
