...
%     delete(p1)
    ...
    X_MPC(:,k) = [X_plant(1,k); X_plant(2,k); X_plant(3,k); s; ey; epsi];
...
title('x');
plot(t_vec, X_plant(4,1:end), 'k', 'DisplayName', 'x');

subplot(m, n, 2); hold on; grid on; 
title('y');
plot(t_vec, X_plant(5,1:end), 'k', 'DisplayName', 'y');

subplot(m, n, 3); hold on; grid on; 
title('psi');
plot(t_vec, X_plant(6,1:end), 'k', 'DisplayName', 'psi');
...
%% Save data
save('trajectory.mat', 'X_MPC', 'X_plant', 'U_MPC', 't_fin')
