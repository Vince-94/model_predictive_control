...
% dvx_wz = vy + (2*Cf*lf*cos(delta_f))/(m*vx);

dvy_vx = 2*((Cf*(vy + lf*wz))*cos(delta_f) + (Cr*(vy - lr*wz)))/(m*vx^2) - wz;
dvy_vy = -2*(Cf*cos(delta_f) + Cr)/(m*vx);
dvy_wz = 2*(Cf*lf*cos(delta_f) - Cr*lr)/(m*vx) - vx;

dwz_vx = 2*(Cf*lf*(vy + lf*wz)*cos(delta_f) - Cr*lr*(vy - lr*wz))/(Iz*vx^2);
dwz_vy = -2*(Cf*lf*cos(delta_f) - Cr*lr)/(Iz*vx);
dwz_wz = -2*(Cf*cos(delta_f)*lf^2 + Cr*lr^2)/(Iz*vx);
...
%% A22
ds_s = 0;
ds_ey = c*(vx*cos(epsi) - vy*sin(epsi))/(1 - c*ey).^2;
ds_epsi = -(vx*sin(epsi) + vy*cos(epsi))/(1 - c*ey)*epsi_dot;
% ds_epsi = (vy*cos(epsi) + vx*sin(epsi))/(c*ey - 1);

dey_s = 0;
dey_ey = 0;
dey_epsi = (vx*cos(epsi) - vy*sin(epsi)) * epsi_dot;
% dey_epsi = vx*cos(epsi) - vy*sin(epsi);

depsi_s = 0;
depsi_ey = -c.^2*(vx*cos(epsi) - vy*sin(epsi))/(1 - c*ey).^2;
depsi_epsi = c*(vx*sin(epsi) + vy*cos(epsi))/(1 - c*ey)*epsi_dot;
% depsi_epsi = -(c*(vy*cos(epsi) + vx*sin(epsi)))/(1 - c*ey);
...
dvy_delta_f = 2*Cf/m*(cos(delta_f) - (delta_f - (vy + lf*wz)/vx)*sin(delta_f));
dvy_a = 0;

dwz_delta_f = 2*Cf*lf/m*(cos(delta_f) - (delta_f - (vy + lf*wz)/vx)*sin(delta_f));
dwz_a = 0;
