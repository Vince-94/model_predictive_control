function U_star = yalmip_mpc(x0, ref, u_old, Np, Q, R, S, prediction_model, x_c, u_c, du_c, J_inf, SS)
...
J_stage = 0;
for i = 1:Np
    J_stage = J_stage + (x{i}-ref)'*Q*(x{i}-ref) + u{i}'*R*u{i} + (u{i}-u_old)'*S*(u{i}-u_old);
end
