...
- [Linear MPC](#linear-mpc)
  ...
  - [State space discretization](#state-space-discretization)
...
- $B \in \mathbb{R}^{n_x, n_u} \quad \text{input-state matrix}$
- $C \in \mathbb{R}^{n_y, n_x} \quad \text{output-state matrix}$
- $D \in \mathbb{R}^{n_y, n_u} \quad \text{feedthrough output matrix}$
...
where:
- $X_k = \begin{bmatrix} x_{0|k}^\top & x_{1|k}^\top & \cdots & x_{N_p-1|k}^\top \end{bmatrix}^\top \in \mathbb{R}^{n_x N_p} \\$
